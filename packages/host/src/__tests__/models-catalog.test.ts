...
describe("control models catalog", () => {
  ...
  it("marks models absent from getAvailable() as unavailable", () => {
    const a = model("openai", "here");
    const b = model("openai", "gone");
    const c = listCatalog(mkRegistry([a, b], [a]));
    expect(c.find((m: any) => m.id === "gone")?.available).toBe(false);
    expect(c.find((m: any) => m.id === "here")?.available).toBe(true);
  });
