...
describe("curator decay", () => {
  it("active→stale after staleAfterDays, →archived after archiveAfterDays", () => {
    ...
    const r = runCuratorDecay(ctx.repoDb, s, now, CURATOR_DEFAULTS);
  ...
  it("min-interval gate blocks a second run inside the window", () => {
    ...
    const s = new SkillStore(ctx.repoDb);
    const now = Date.now();
    runCuratorDecay(ctx.repoDb, s, now, CURATOR_DEFAULTS);
    expect(curatorShouldRun(ctx.repoDb, now + 1000, CURATOR_DEFAULTS)).toBe(false); // < 24h
    expect(curatorShouldRun(ctx.repoDb, now + 25 * 3600 * 1000, CURATOR_DEFAULTS)).toBe(true); // > 24h
  });
});
